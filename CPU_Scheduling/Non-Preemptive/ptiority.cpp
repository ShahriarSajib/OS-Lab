...
struct process {
    string id;
    int arrival, burst, priority, waiting = 0, turnaround = 0;
    bool done = false;
};
...
int main() {
    ...
    cin >> n;
    vector<process> p(n);
    for (auto &x : p) cin >> x.id >> x.arrival >> x.burst >> x.priority;
    ...
    while (done < n) {
        ...
        for (int i = 0; i < n; i++)
            if (!p[i].done && p[i].arrival <= time)
                if (idx == -1 || p[i].priority < p[idx].priority) idx = i;
    ...
    for (auto &x : p) cout << x.id << "\t" << x.waiting << "\t" << x.turnaround << "\n";
}
